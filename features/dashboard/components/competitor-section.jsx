...
function resolveCompetitorBannerLogo(store, linkedBanner) {
  return (
...
export default function CompetitorSection({
  ...
  return (
    <SectionCard
      title="CONCORRENTES"
      full
      hint="Lista de lojas concorrentes da conta ativa."
    >
      {!tenantId ? <div className={"rounded-lg border border-dashed border-slate-300 p-2.5 text-xs opacity-75"}>Selecione uma conta ativa para gerenciar concorrentes.</div> : null}
      ...
      <div className={"grid gap-1.5"}>
        {stores.length === 0 ? (
          <div className={"rounded-lg border border-dashed border-slate-300 p-2.5 text-xs opacity-75"}>Nenhuma loja concorrente cadastrada.</div>
        ) : (
          stores.map((store) => {
            const linkedBanner = bannerMap[String(store.banner_id)] || null;
            const storeLogo = resolveCompetitorBannerLogo(store, linkedBanner);
            const competitorBannerName =
            ...
            return (
              <article key={store.id} className={"grid gap-1.5 rounded-lg border border-slate-200 bg-white p-2"}>
                ...
                <small>
                  Rede: {networkName[String(store.network_id)] || "N/A"} | Bandeira: {competitorBannerName}
                </small>
                <small>
